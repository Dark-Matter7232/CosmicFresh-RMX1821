...
#endif /* PT_MTK */
#ifdef CONFIG_PM_WAKELOCKS
static struct wakeup_source ilitek_gestrue_wakelock;
#else
...
static unsigned char fw_ili_inx[] = {
	#include "core/FW_TDDI_9881H_INX.ili"
};
...
struct ili_model_file ili_mode_file_info[] = {
	...
	{.mp_ini_name = MP_INI_NAME_INX, .fw_bin_name = FW_BIN_NAME_INX, .fw_bin_sign_name = FW_BIN_SIGN_NAME_INX, .fw_ili_code = fw_ili_inx, .fw_ili_size = ARRAY_SIZE(fw_ili_inx) },
};
...
static irqreturn_t ilitek_platform_irq_handler(int irq, void *dev_id)
{
	...
	if (core_gesture->suspend) {
#ifdef CONFIG_PM_WAKELOCKS
		__pm_wakeup_event(&ilitek_gestrue_wakelock, 2500);
#else
...
#endif
{
	...
	}else if (ili_ctpmodule == INX_MODEL) {
       ipd->fw_bin_name = ili_mode_file_info[INX_MODEL].fw_bin_name;
       ipd->fw_bin_sign_name = ili_mode_file_info[INX_MODEL].fw_bin_sign_name;
       ipd->mp_ini_name = ili_mode_file_info[INX_MODEL].mp_ini_name;
       ipd->ili_fw_size = ili_mode_file_info[INX_MODEL].fw_ili_size;
       CTPM_FW = (uint8_t*)vmalloc(ipd->ili_fw_size);
       if (ERR_ALLOC_MEM(CTPM_FW)) {
               ipio_err("Failed to allocate CTPM_FW, %ld\n", PTR_ERR(CTPM_FW));
               return -ENOMEM;
       }
       CTPM_FW = ili_mode_file_info[INX_MODEL].fw_ili_code;
	}
	else {
...
*/
	...
	ipd->pm_suspend = false;  //mfeng_add
	init_completion(&ipd->pm_completion);	//mfeng_add
#ifdef MING_TEST
...
#ifdef CONFIG_PM_WAKELOCKS
	wakeup_source_init(&ilitek_gestrue_wakelock, "poll-wake-lock");
#else
...
//Alloc fw_name/devinfo memory space
	...
	if (core_config->ilitek_ts_info.fw_path == NULL) {
		...
		if(ili_ctpmodule == INX_MODEL)
			core_config->ilitek_ts_info.fw_path =  FW_BIN_SIGN_NAME_INX;
	}
...
ipio_debug_level =0;
        ...
        reload_fw_ws = wakeup_source_register("ili_wakelock");
	if(!reload_fw_ws)
...
static int ilitek_tp_pm_suspend(struct device *dev)
{
	...
	ipd->pm_suspend = true;
	reinit_completion(&ipd->pm_completion);
	return 0;
}
...
static int ilitek_tp_pm_resume(struct device *dev)
{
	...
	ipd->pm_suspend = false;
	complete(&ipd->pm_completion);
	return 0;
}
...
/* YunRui.Chen@RM.BSP.TP.FUNCTION, 2018/12/7, Add for cts gsi test  add end */	
	...
	else if(ret1 == 1&&ret2 == 0 && ret3 ==0) {
		ili_ctpmodule = INX_MODEL;
	}
