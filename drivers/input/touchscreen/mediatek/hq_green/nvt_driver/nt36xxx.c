...
//cbs
	...
	if (strstr(saved_command_line, "nt36672c_jdi"))
		nvt_update_firmware(BOOT_UPDATE_FIRMWARE_NAME_JDI);
	if (strstr(saved_command_line, "nt36672c_boe"))
...
//cbs
	if (strstr(saved_command_line, "nt36672c_tianma")) {
		if (nvt_update_firmware(BOOT_UPDATE_FIRMWARE_NAME_TM)) {
	...
	if (strstr(saved_command_line, "nt36672c_jdi")) {
		if (nvt_update_firmware(BOOT_UPDATE_FIRMWARE_NAME_JDI)) {
			NVT_ERR("download firmware failed, ignore check fw state\n");
		} else {
			nvt_check_fw_reset_state(RESET_STATE_REK);
		}
...
static int32_t __init nvt_driver_init(void)
{
	...
	ret1 = gpio_get_value(359);
//	ret = gpio_get_value(LCD_PHY_GPIO);
	NVT_LOG("ret1: %d\n",ret1);
	NVT_LOG("ret1: %d\n",ret1);
	if(ret1 == 0) {
		NVT_LOG("this is CPHY NT36672C LCD\n");
	}
	else  {
		NVT_LOG("ret = %d, this is not CPHY NT36672C LCD\n",ret1);
		return 0;
	}
